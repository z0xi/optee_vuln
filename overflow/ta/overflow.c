...
{
	DMSG("=====================================================");
	DMSG("Backdoor is called!\n");
	DMSG("=====================================================");
	return TEE_SUCCESS;
}

static TEE_Result show_backdoor(uint32_t param_types,
	TEE_Param params[4])
{
	uint32_t exp_param_types = TEE_PARAM_TYPES
        (
	...
	DMSG("=====================================================");
	DMSG("Hello, I can leak backdoor pointer!\n");
	// No check
	if (param_types != exp_param_types)
    {
		DMSG("No check!\n");
	} // return TEE_ERROR_BAD_PARAMETERS;
	DMSG("backdoor is at %p", (void*)backdoor);
	// DMSG("TEE_ReadObjectData is at %p", (void*)TEE_ReadObjectData);
	params[0].value.a = (int)backdoor;
			...
			uint32_t param_types, TEE_Param params[4])
{
	...
	switch (cmd_id) {
	case TA_OVERFLOW_CMD:
		return call_fibufnacci(param_types, params);
	case TA_BACKDOOR_CMD:
		return show_backdoor(param_types, params);
	default:
		return TEE_ERROR_BAD_PARAMETERS;
	}
}
